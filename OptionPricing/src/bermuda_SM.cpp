...
#pragma omp for schedule(static)
            for (int block = 0; block < b / buffer; block++)
            {
                ...
                for (int g_idx = 0; g_idx < buffer * stock_count; g_idx++)
                {
                    if (has_spare)
                    {
                ...
                for (int i = 0; i < buffer; i++)
                {
                    for (int j = 0; j < stock_count; j++)
                    {
                        ...
                        double e = CUSTOM_EXP(drift[j] * dt[cur_t] + d);
...
#pragma omp for schedule(static)
            for (int next_node = 0; next_node < b; next_node++)
            {
                ...
                for (int cur_node = 0; cur_node < b; cur_node++)
                {
                    ...
                    for (int st = 0; st < stock_count; st++)
                    {
                        double alpha = CUSTOM_LOG(nodes[(cur_t + 1) * b * stock_count + (next_node * stock_count) + st] / nodes[(cur_t)*b * stock_count + (cur_node * stock_count) + st]) - drift[st] * dt[cur_t];
...
#pragma omp for schedule(static)
            for (int cur_node = 0; cur_node < b; cur_node++)
            {
                ...
                high_est[cur_t * b + cur_node] *= CUSTOM_EXP(-interest_rate * dt[cur_t]);
        ...
        for (int cur_path = 0; cur_path < b; cur_path++)
        {
            for (int cur_t = 1; cur_t < exercise_num; cur_t++)
            {
                ...
                for (int next_node = 0; next_node < b; next_node++)
                {
                    ...
                    for (int st = 0; st < stock_count; st++)
                    {
                        double alpha = CUSTOM_LOG(nodes[(cur_t + 1) * b * stock_count + (next_node * stock_count) + st] / nodes_low[(cur_t)*b * stock_count + (cur_path * stock_count) + st]) - drift[st] * dt[cur_t];
                        cur_TD *= CUSTOM_EXP(-(alpha * alpha) / (vol_sq_dt[(cur_t + 1) * stock_count + st])) * sqrt_pi_vol[(cur_t + 1) * stock_count + st];
                    }
                    next_est += (high_est[(cur_t + 1) * b + next_node] * cur_TD) / denom[(cur_t + 1) * b + next_node];
                }
                next_est /= b;
                next_est *= CUSTOM_EXP(-interest_rate * dt[cur_t]);
                if (ex_price >= next_est)
                {
                    low_est[cur_path] = ex_price * CUSTOM_EXP(-interest_rate * exercise_dates[cur_t - 1]);
                    break;
                }
            }
            if (low_est[cur_path] < 0)
            {
                ...
                low_est[cur_path] = mx * CUSTOM_EXP(-interest_rate * exercise_dates[exercise_num - 1]);
            }
        }
